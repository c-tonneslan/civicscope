...
def overview() -> OverviewResponse:
    ...
    return OverviewResponse(**corpus_overview())


@router.get("/topics", response_model=TopicActivityResponse)
def topics(since: date | None = None) -> TopicActivityResponse:
    """Bill counts for curated policy topics, optionally since ``?since=YYYY-MM-DD``."""

    from app.civic.insights import topic_activity

    return TopicActivityResponse(**topic_activity(since))
